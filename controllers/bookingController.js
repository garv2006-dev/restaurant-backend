...
const checkOutBooking = async (req, res) => {
    try {
        ...
        try {
            const loyaltyResult = await awardPointsForBooking(booking.user, booking.pricing.totalAmount);
            if (loyaltyResult) {
                console.log(`Awarded ${loyaltyResult.pointsAwarded} loyalty points to user ${booking.user}`);
            }
        } catch (loyaltyError) {
